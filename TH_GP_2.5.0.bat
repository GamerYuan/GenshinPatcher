...
REM Check if all files listed in TH_GP_AudioPatch_2.5.0.txt are present.
...
for /F "usebackq delims=" %%i in ("!WorkingDirForAdmin!TH_GP_AudioPatch_2.5.0.txt") do (
...
for /F "usebackq delims=" %%i in ("!WorkingDirForAdmin!TH_GP_Cleanup_2.5.0.txt") do (
	if "%%i"=="mhypbase.dll" (
...
REM Delete source file for TH_GP_Cleanup_2.5.0.txt and the file itself.
del "!WorkingDirForAdmin!deletefiles.txt"
del "!WorkingDirForAdmin!TH_GP_Cleanup_2.5.0.txt"
REM Delete obsolete .hdiff files after patch application.
for /F "usebackq delims=" %%i in ("!WorkingDirForAdmin!TH_GP_AudioPatch_2.5.0.txt") do (
    del "!WorkingDirForAdmin!%%i.hdiff"
)
REM Delete source file for TH_GP_AudioPatch_2.5.0.txt and the file itself.
del "!WorkingDirForAdmin!hdifffiles.txt"
del "!WorkingDirForAdmin!TH_GP_AudioPatch_2.5.0.txt"
...
if "%PatchFinished%"=="True" (
    del "!WorkingDirForAdmin!TH_GP_2.5.0.bat"
)