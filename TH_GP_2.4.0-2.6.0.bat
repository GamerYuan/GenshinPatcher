...
REM Check if all files listed in TH_GP_AudioPatch_2.4.0-2.6.0.txt are present.
echo Checking if all necessary files for Patch 2.4-2.6 are present...
...
for /F "usebackq delims=" %%i in ("!WorkingDirForAdmin!TH_GP_AudioPatch_2.4.0-2.6.0.txt") do (
...
for /F "usebackq delims=" %%i in ("!WorkingDirForAdmin!TH_GP_Cleanup_2.4.0-2.6.0.txt") do (
	if "%%i"=="mhypbase.dll" (
...
REM Delete source file for TH_GP_Cleanup_2.4.0-2.6.0.txt and the file itself.
del "!WorkingDirForAdmin!deletefiles.txt"
del "!WorkingDirForAdmin!TH_GP_Cleanup_2.4.0-2.6.0.txt"
REM Delete obsolete .hdiff files after patch application.
for /F "usebackq delims=" %%i in ("!WorkingDirForAdmin!TH_GP_AudioPatch_2.4.0-2.6.0.txt") do (
    del "!WorkingDirForAdmin!%%i.hdiff"
)
REM Delete source file for TH_GP_AudioPatch_2.4.0-2.6.0.txt and the file itself.
del "!WorkingDirForAdmin!hdifffiles.txt"
del "!WorkingDirForAdmin!TH_GP_AudioPatch_2.4.0-2.6.0.txt"
...
if "%PatchFinished%"=="True" (
    del "!WorkingDirForAdmin!TH_GP_2.4.0-2.6.0.bat"
)